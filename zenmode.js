...
window.onSpotifyWebPlaybackSDKReady = () => {
  ...
  player.connect().then((success) => {
    if (success) {
      ...
      // Optionally, you can redirect the user or show a message here
    }
  });
...
async function refreshAccessToken() {
  ...
  try {
    const response = await fetch(`http://127.0.0.1:3000/refresh_token?refresh_token=${refreshToken}`);
    const data = await response.json();
    if (data.access_token) {
      window.location.search = `?access_token=${data.access_token}&refresh_token=${refreshToken}`;
    }
  } catch (error) {
...
// Redirect to Spotify login if the user is not authenticated
document.getElementById('spotify-icon').addEventListener('click', () => {
  window.location.href = 'http://127.0.0.1:3000/login'; // Redirect to Spotify login
});
